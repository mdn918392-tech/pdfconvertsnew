...
import { Download, ArrowLeft, FileText, Scissors, Eye, Filter, Shield, Zap, CheckCircle, Sparkles, File, X, Check, Clock } from 'lucide-react';
...
export default function ExtractPages() {
  ...
  const generateUniqueFileName = (baseName: string, pages: number[]) => {
    const timestamp = new Date().getTime();
    const randomId = Math.random().toString(36).substring(2, 9);
    const cleanBaseName = baseName.replace(/\.[^/.]+$/, ""); // Remove extension
    const pageList = pages.join('-');
    return `${cleanBaseName}_pages_${pageList}_${timestamp}_${randomId}.pdf`;
  };
  ...
  const handleDownload = () => {
    if (pdfBlob) {
      const pages = parsePageNumbers(pageNumbers, totalPages);
      const fileName = generateUniqueFileName(files[0].name, pages);
  ...
  // Render PDF preview using iframe - MOBILE FIXED
  const renderPdfPreview = () => {
    if (!pdfUrl || !isClient) {
      return (
        <div className="w-full h-full flex flex-col items-center justify-center p-4">
          <FileText className="w-12 h-12 text-gray-400 mb-3" />
          <span className="text-gray-500 text-center">Preview not available</span>
        </div>
      );
    }
  ...
  // Render full screen preview modal - MOBILE FIXED
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs opacity-80 mb-2">
                    {notification.pageCount} pages extracted
                  </p>
        ...
        {previewModal.open && previewModal.url && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1 }}
              exit={{ scale: 0.9 }}
              className="relative w-full max-w-6xl max-h-[90vh] bg-gradient-to-br from-gray-800 to-gray-900 rounded-xl sm:rounded-2xl overflow-hidden shadow-2xl"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Modal Header */}
              <div className="flex items-center justify-between p-3 sm:p-4 bg-gradient-to-r from-blue-600 to-purple-600">
                <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                  <Eye className="w-4 h-4 sm:w-5 sm:h-5 text-white flex-shrink-0" />
                  <div className="min-w-0">
                    ...
                    {pdfBlob && (
                      <p className="text-xs text-white/80 truncate">
                        {extractedPages.length} pages • {(pdfBlob.size / 1024 / 1024).toFixed(2)} MB
                      </p>
                    )}
                  </div>
                </div>
          ...
          >
            ...
            <div className="mb-6 sm:mb-8 md:mb-12">
              ...
              <div className="text-center mb-4 sm:mb-6 md:mb-8">
                <motion.div 
                  initial={{ scale: 0.5 }}
                  animate={{ scale: 1 }}
                  className="inline-flex items-center justify-center w-14 h-14 sm:w-16 sm:h-16 md:w-20 md:h-20 bg-gradient-to-br from-blue-500 to-purple-600 rounded-2xl sm:rounded-3xl mb-3 sm:mb-6 shadow-2xl"
                >
                  <Scissors className="w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 text-white" />
                </motion.div>
                
                <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black text-gray-900 dark:text-white mb-2 sm:mb-4 bg-gradient-to-r from-blue-600 via-purple-600 to-blue-600 bg-clip-text text-transparent px-2">
                  PDF Page Extractor
                </h1>
                  ...
                  {[
                    {
                      ...
                      desc: "Extract specific pages or page ranges from your PDF",
                    ...
                    },
                    {
                      ...
                      desc: "Extract pages instantly with optimized algorithms",
            ...
            <div className="bg-white dark:bg-gray-900 rounded-lg sm:rounded-xl md:rounded-2xl lg:rounded-3xl border-2 border-gray-200 dark:border-gray-800 shadow-lg sm:shadow-xl md:shadow-2xl p-3 sm:p-4 md:p-6 lg:p-8 mb-6 md:mb-8">
              ...
              <div className="mb-4 sm:mb-6 md:mb-8">
                ...
                {files.length > 0 && (
                  <div className="mt-3 text-center">
                    <div className="inline-flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 px-3 py-1.5 sm:px-4 sm:py-2 bg-gradient-to-r from-blue-100 to-purple-100 dark:from-blue-900/30 dark:to-purple-900/30 rounded-lg sm:rounded-full text-xs sm:text-sm">
                      <div className="flex items-center gap-1 sm:gap-2">
                        <FileText className="w-3 h-3 sm:w-4 sm:h-4 text-blue-600 dark:text-blue-400" />
                        <span className="font-medium text-blue-700 dark:text-blue-300 truncate max-w-[200px] sm:max-w-none">
                          {files[0].name}
                        </span>
                      </div>
              ...
              {files.length > 0 && (
                <div className="space-y-4 sm:space-y-6 md:space-y-8">
                  ...
                  <div className="p-3 sm:p-4 md:p-6 bg-gradient-to-r from-gray-50 to-blue-50 dark:from-gray-800 dark:to-blue-950/20 rounded-lg sm:rounded-xl md:rounded-2xl border-2 border-blue-200 dark:border-blue-800/30">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-3 md:gap-4">
                      <div className="flex items-center gap-2 sm:gap-3 md:gap-4">
                        <div className="p-1.5 sm:p-2 md:p-3 bg-gradient-to-r from-blue-500 to-purple-600 rounded-lg sm:rounded-xl">
                          <FileText className="w-4 h-4 sm:w-5 sm:h-5 md:w-8 md:h-8 text-white" />
                        </div>
                        <div>
                          <h3 className="font-bold text-gray-900 dark:text-white text-sm sm:text-base md:text-lg break-words line-clamp-2 max-w-[180px] sm:max-w-[250px] md:max-w-[350px]">
                            {files[0].name}
                          </h3>
                  ...
                  <div className="sm:hidden p-2 sm:p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                    <p className="text-xs text-blue-700 dark:text-blue-300 text-center">
                      👆 Tap on PDF preview to view clearly
                    </p>
                  </div>
                  ...
                  <div className="space-y-3 sm:space-y-4 md:space-y-6">
                    <h3 className="text-base sm:text-lg md:text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                      <Scissors className="w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5 text-blue-500" />
                    ...
                    <div className="space-y-2 sm:space-y-3 md:space-y-4">
                      <div>
                        ...
                        <div className="relative">
                          <input
                            ...
                            className="w-full px-3 py-2.5 sm:px-4 sm:py-3 md:px-6 md:py-4 border-2 border-gray-300 dark:border-gray-700 rounded-lg sm:rounded-xl md:rounded-2xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all text-sm sm:text-base"
                          />
                          <div className="absolute right-2 sm:right-3 top-1/2 transform -translate-y-1/2">
                            {extractedPages.length > 0 && (
                              <span className="text-xs font-semibold text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-950/30 px-1.5 sm:px-2 py-0.5 rounded-full">
                      ...
                      {extractedPages.length > 0 && (
                        <motion.div
                          ...
                          animate={{ opacity: 1, y: 0 }}
                          className="p-2 sm:p-3 md:p-4 bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-950/30 dark:to-purple-950/30 rounded-lg sm:rounded-xl md:rounded-2xl border-2 border-blue-200 dark:border-blue-800/50"
                        >
                          <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 md:gap-3">
                            <div className="flex items-center gap-1 sm:gap-2">
                              <div className="p-1 sm:p-1.5 bg-gradient-to-r from-blue-500 to-purple-600 rounded-lg">
                                <File className="w-3 h-3 sm:w-3 sm:h-3 md:w-4 md:h-4 text-white" />
                              </div>
                            ...
                            <div className="flex-1 mt-1 sm:mt-0">
                              <div className="flex flex-wrap gap-1 sm:gap-1.5 md:gap-2">
                                {extractedPages.map((page, index) => (
                                  <span
                                    key={page}
                                    className="px-1.5 sm:px-2 md:px-3 py-0.5 sm:py-1 bg-gradient-to-r from-blue-600 to-purple-600 text-white text-xs sm:text-xs md:text-sm rounded-md sm:rounded-lg font-medium shadow-sm"
                                  >
                                    Page {page}
                                  </span>
                                ))}
                  ...
                  <div className="space-y-3 sm:space-y-4">
                    {converting && (
                      <div className="space-y-2 sm:space-y-3 md:space-y-4">
                        ...
                        />
                        <div className="flex items-center justify-center gap-1 sm:gap-2 text-blue-600 dark:text-blue-400">
                    ...
                    {!pdfBlob && !converting && (
                      <motion.button
                        ...
                        className={`w-full py-2.5 sm:py-3 md:py-4 px-3 sm:px-4 md:px-6 font-bold rounded-lg sm:rounded-xl md:rounded-2xl shadow-md sm:shadow-lg md:shadow-xl hover:shadow-2xl transition-all text-sm sm:text-base md:text-lg flex items-center justify-center gap-1.5 sm:gap-2 md:gap-3 ${
                          extractedPages.length === 0
                            ? 'bg-gray-400 dark:bg-gray-600 cursor-not-allowed text-white'
                            : 'bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white'
                        }`}
                      >
                        <Scissors className="w-3.5 h-3.5 sm:w-4 sm:h-4 md:w-6 md:h-6" />
                        {extractedPages.length === 0 ? 'Enter Page Numbers' : `Extract ${extractedPages.length} Pages`}
                        <Zap className="w-3 h-3 sm:w-3 sm:h-3 md:w-5 md:h-5" />
                      </motion.button>
                    )}
                  </div>
                    ...
                    >
                      ...
                      <div className="p-3 sm:p-4 md:p-6 bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-950/30 dark:to-emerald-950/30 rounded-lg sm:rounded-xl md:rounded-2xl border-2 border-green-200 dark:border-green-800/50">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 md:gap-4">
                          <div className="flex items-center justify-center sm:justify-start">
                            <div className="p-1.5 sm:p-2 md:p-3 bg-gradient-to-r from-green-500 to-emerald-600 rounded-lg sm:rounded-xl">
                              <CheckCircle className="w-5 h-5 sm:w-6 sm:h-6 md:w-8 md:h-8 text-white" />
                            </div>
                          </div>
                          <div className="flex-1 text-center sm:text-left">
                            <h3 className="text-sm sm:text-base md:text-lg lg:text-xl font-bold text-gray-900 dark:text-white mb-1">
                            ...
                            <p className="text-gray-600 dark:text-gray-400 text-xs sm:text-xs md:text-sm mt-0.5">
                      ...
                      <div className="space-y-2 sm:space-y-3 md:space-y-4">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                          <h4 className="text-sm sm:text-base md:text-lg font-bold text-gray-900 dark:text-white flex items-center gap-1.5 sm:gap-2">
                            <Eye className="w-3.5 h-3.5 sm:w-4 sm:h-4 md:w-5 md:h-5 text-blue-500" />
                            Preview Extracted PDF
                          </h4>
                          <div className="flex gap-1.5 sm:gap-2">
                            <button
                              onClick={() => setExpandedPreview(!expandedPreview)}
                              className="px-2 py-1 sm:px-3 sm:py-1.5 text-xs sm:text-sm font-medium text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/30 hover:bg-blue-100 dark:hover:bg-blue-900/50 rounded-lg sm:rounded-xl transition-colors"
                            >
                              {expandedPreview ? 'Minimize' : 'Expand'}
                            </button>
                            <button
                              onClick={() => setPreviewModal({url: pdfUrl, open: true})}
                              className="px-2 py-1 sm:px-3 sm:py-1.5 text-xs sm:text-sm font-medium text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-950/30 hover:bg-purple-100 dark:hover:bg-purple-900/50 rounded-lg sm:rounded-xl transition-colors"
                            >
                              Full Screen
                            </button>
                          </div>
                          ...
                          {previewLoading ? (
                            <div className="w-full h-full flex items-center justify-center">
                              <div className="flex flex-col items-center gap-2">
                                ...
                                <span className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">Loading preview...</span>
                              </div>
                            </div>
                          ) : renderPdfPreview()}
                        </div>
            ...
            <div className="mt-4 sm:mt-8 md:mt-12 pt-3 sm:pt-6 md:pt-8 border-t border-gray-200 dark:border-gray-800">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 md:gap-4 lg:gap-6 text-center">
                {[
                  { value: files.length > 0 ? '✓' : '—', label: 'File Uploaded', color: 'text-blue-600' },
                  { value: files.length > 0 ? totalPages : '—', label: 'Total Pages', color: 'text-purple-600' },
                  { value: extractedPages.length, label: 'Pages Selected', color: 'text-green-600' },
                  { value: pdfBlob ? '✓' : '—', label: 'Ready to Download', color: 'text-emerald-600' }
                ].map((stat, index) => (
                  <div key={index}>
