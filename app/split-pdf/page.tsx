...
const getToolDescription = (label: string): string => {
  const lowerLabel = label.toLowerCase();
  if (lowerLabel.includes("split") || lowerLabel.includes("splitter"))
    return "Split PDF into individual pages and download them separately.";
  if (lowerLabel.includes("rotate") || lowerLabel.includes("rotator"))
      ...
      <div className="absolute top-2 left-2 z-30">
        <button
          ...
          className={`p-1.5 rounded-full backdrop-blur-sm transition-colors ${
            selected ? "bg-blue-600" : "bg-black/70 hover:bg-black/90"
          }`}
    ...
    >
      ...
      <div className="absolute bottom-20 left-1/2 transform -translate-x-1/2 z-50">
        <div className="bg-black/70 text-white text-xs px-3 py-2 rounded-full backdrop-blur-sm">
          <span>
            Scroll to zoom • Use rotation buttons • Click outside to close
          </span>
        </div>
      </div>
    </motion.div>
...
const generateSplitPdfFilename = (
...
export default function PdfSplitRotatorTool() {
  ...
  const handleRotationChange = async (
    ...
    if (pageInfo) {
      ...
      updatedPageData[pageIndex] = {
        ...
        fileName: generateSplitPdfFilename(
          files[0].name,
          pageInfo.pageNumber,
          pageData.length,
          newRotation
      ...
      if (
        simpleZoomModal.isOpen &&
        simpleZoomModal.pageNumber === pageInfo.pageNumber
      ) {
  ...
  const handleDownloadSelected = async () => {
    ...
    try {
      ...
      // Success message
      setDownloadSuccess(
        `✓ Successfully downloaded ${selectedPages.length} selected pages!`
      );
      setTimeout(() => setDownloadSuccess(null), 5000);
    } catch (error) {
      console.error("Error downloading selected pages:", error);
      setDownloadSuccess(
        "✗ Failed to download selected pages. Please try again."
      );
      setTimeout(() => setDownloadSuccess(null), 3000);
    } finally {
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      ...
      // Success message
      setDownloadSuccess(
        `✓ Successfully downloaded all ${pageData.length} pages!`
      );
      setTimeout(() => setDownloadSuccess(null), 5000);
    } catch (error) {
  ...
  const handleDownloadCurrentPage = async () => {
    ...
    try {
      ...
      setDownloadSuccess(
        `✓ Successfully downloaded ${endIndex - startIndex} pages!`
      );
      setTimeout(() => setDownloadSuccess(null), 3000);
    } catch (error) {
   ...
   {/* SEO Schema */}
    ...
    mb-3 md:mb-4 shadow-xl`}
            ...
            <p className="text-base sm:text-lg md:text-xl text-gray-600 dark:text-gray-400 max-w-3xl mx-auto leading-relaxed px-2">
              Split PDF into individual pages or page ranges and download them instantly.
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 md:gap-4">
                          ...
                          <div className="flex-1 text-center sm:text-left">
                            ...
                            <p className="text-green-700 dark:text-green-300 font-medium text-xs sm:text-sm md:text-base">
                              {pageData.length} pages ready for splitting and
                              rotation
                            </p>
                            <p className="text-gray-600 dark:text-gray-400 text-xs sm:text-sm mt-1">
                              Select pages, rotate them, and download
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 md:gap-6">
                          {currentPageData.map((page, index) => {
                            ...
                            return (
                              ...
                              >
                                <div
                                  className={`bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-xl sm:rounded-2xl border-2 p-3 sm:p-4 md:p-5 shadow-lg hover:shadow-xl transition-all duration-300 ${
                                    page.selected
